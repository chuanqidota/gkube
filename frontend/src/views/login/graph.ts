...
export function useClusterGraph(canvasRef: Ref<HTMLCanvasElement | undefined>) {
  ...
  function step(ts: number) {
    ...
    for (const n of NODES) {
      ...
      if (n.kind === 'cluster') {
        ...
        ctx.fillStyle = hi > 0.01 ? (isError ? PALETTE.error : PALETTE.workerHi) : PALETTE.worker
        ctx.fill()
        ctx.shadowBlur = 0
        ctx.lineWidth = 1
        ctx.strokeStyle =
  ...
  function renderStatic() {
    ...
    for (const e of EDGES) {
      ...
      ctx.lineWidth = 1 + hi * 1.2
      ctx.strokeStyle =
        hi > 0.01 ? `rgba(129, 140, 248, ${(0.18 + hi * 0.7).toFixed(3)})` : PALETTE.edgeDim
      ctx.beginPath()
      ctx.moveTo(a.x, a.y)
      ctx.quadraticCurveTo(c.x, c.y, b.x, b.y)
      ctx.stroke()
  ...
  function destroy() {
    ...
    ctx = null
    // Reset singleton highlight state so a re-mount starts clean
    for (const n of NODES) {
      n.hi = 0
      n.hiTarget = 0
    }
