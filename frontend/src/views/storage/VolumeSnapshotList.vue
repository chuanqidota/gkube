...
function handleDetail(row: any) {
  router.push(`/storage/volumesnapshots/${row.namespace}/${row.name}`)
}
...
<template>
  <div class="page-container">
    ...
    >
      <template #actions>
        <el-button type="success" @click="router.push('/storage/volumesnapshots/create')"
    ...
    <el-card shadow="never" class="table-card">
      ...
      >
        ...
        <el-table-column :label="t('common.status')" width="120">
          <template #default="{ row }"
            ><el-tag :type="statusType(getStatus(row))" size="small">{{
              getStatus(row)
            }}</el-tag></template
          >
        </el-table-column>
    ...
    <YamlDrawer
      ...
      :get-yaml="getVolumeSnapshotYaml"
      :update-yaml="updateVolumeSnapshot"
      :namespace="yamlTarget?.namespace || ''"
      :name="yamlTarget?.name || ''"
      title="VolumeSnapshot YAML"
