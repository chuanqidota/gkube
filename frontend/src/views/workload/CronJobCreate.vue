...
import { createCronJob, getCronJobList, getCronJobYaml } from '@/api/resource'
...
<template>
  <div class="cronjob-create">
    ...
    <!-- CronJobForm 始终挂载（v-show），避免克隆后切换 mode 时组件被销毁重建导致数据丢失 -->
    <CronJobForm v-show="mode === 'form'" :initial-data="parsedData" />
