...
async function handleViewYaml(row: any) {
  ...
  yamlContent.value = ''
  try {
    const res: any = await getDaemonSetYaml({
...
async function handleDelete(row: any) {
  try {
    await ElMessageBox.confirm(
      `Delete daemonset "${row.name}" in namespace "${row.namespace}"?`,
      'Confirm',
      { type: 'warning' }
    )
    await deleteDaemonSet({ clusterName: clusterName.value, namespace: row.namespace, name: row.name })
...
<template>
  <div>
    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 16px;">
      ...
      <div style="display: flex; gap: 12px; align-items: center;">
        <el-input
          ...
          style="width: 180px;"
          clearable
          @clear="fetchDaemonSets"
          @keyup.enter="fetchDaemonSets"
        />
        <el-select
    ...
    <el-table :data="daemonSetList" v-loading="loading" stripe style="width: 100%">
      ...
      <el-table-column label="Actions" width="260" fixed="right">
        <template #default="{ row }">
          <el-button size="small" @click="router.push(`/workloads/daemonsets/${row.namespace}/${row.name}?cluster=${clusterName}`)">Detail</el-button>
