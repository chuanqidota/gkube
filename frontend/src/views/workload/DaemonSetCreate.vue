...
import { createDaemonSet } from '@/api/resource'
...
async function handleYamlSubmit() {
  ...
  try {
    ...
    await createDaemonSet({ namespace: ns, yaml: yamlContent.value })
    ElMessage.success('DaemonSet created successfully')
...
<template>
  <div class="daemonset-create">
    ...
    <WorkloadForm v-if="mode === 'form'" kind="DaemonSet" />

    <div v-else class="yaml-mode">
      <div class="form-header"><h2>Create DaemonSet (YAML)</h2></div>
      <YamlEditor v-model="yamlContent" height="calc(100vh - 230px)" :read-only="false" editable auto-format />
      <div class="form-actions">
        <el-button @click="handleCancel">Cancel</el-button>
        <el-button type="primary" :loading="submitting" @click="handleYamlSubmit">Create DaemonSet</el-button>
      </div>
    </div>
  </div>
</template>
...
<style scoped>
.daemonset-create { max-width: 1100px; margin: 0 auto; padding: 20px 0; }
