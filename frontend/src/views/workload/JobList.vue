...
async function handleViewYaml(row: any) {
  ...
  yamlContent.value = ''
  try {
    const res: any = await getJobYaml({
...
async function handleDelete(row: any) {
  try {
    await ElMessageBox.confirm(
      `Delete job "${row.name}" in namespace "${row.namespace}"?`,
      'Confirm',
      { type: 'warning' }
    )
    await deleteJob({ clusterName: clusterName.value, namespace: row.namespace, name: row.name })
    ...
    <el-table :data="jobList" v-loading="loading" stripe style="width: 100%">
      ...
      <el-table-column label="Actions" width="260" fixed="right">
        <template #default="{ row }">
          <el-button size="small" @click="router.push(`/workloads/jobs/${row.namespace}/${row.name}?cluster=${clusterName}`)">Detail</el-button>
