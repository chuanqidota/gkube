...
function parseInitialData(data: any) {
  ...
  const parseAffinityRules = (rules: any[]): AffinityRule[] => {
    return (rules || []).map((r: any) => ({
      ...
      labelKey: r.podAffinityTerm?.labelSelector?.matchLabels
        ? Object.keys(r.podAffinityTerm.labelSelector.matchLabels)[0] || ''
        : r.labelSelector?.matchLabels
          ? Object.keys(r.labelSelector.matchLabels)[0] || ''
          : '',
      labelValue: r.podAffinityTerm?.labelSelector?.matchLabels
        ? (Object.values(r.podAffinityTerm.labelSelector.matchLabels)[0] as string) || ''
        : r.labelSelector?.matchLabels
          ? (Object.values(r.labelSelector.matchLabels)[0] as string) || ''
          : '',
    }))
  }
  ...
  if (affinity.podAffinity?.requiredDuringSchedulingIgnoredDuringExecution) {
    form.podAffinityRules.push(
      ...affinity.podAffinity.requiredDuringSchedulingIgnoredDuringExecution.map((r: any) => ({
        ...
        labelKey: r.labelSelector?.matchLabels
          ? Object.keys(r.labelSelector.matchLabels)[0] || ''
          : '',
        labelValue: r.labelSelector?.matchLabels
          ? (Object.values(r.labelSelector.matchLabels)[0] as string) || ''
          : '',
      })),
    )
  ...
  form.topologySpreadConstraints = (podSpec.topologySpreadConstraints || []).map((t: any) => ({
    ...
    labelValue: t.labelSelector?.matchLabels
      ? (Object.values(t.labelSelector.matchLabels)[0] as string) || ''
      : '',
  }))
...
function buildK8sResource(): Record<string, any> {
  ...
  const selectorLabels: Record<string, string> =
    props.isEdit && props.initialData?.spec?.selector?.matchLabels
  ...
  function buildContainer(c: Container): Record<string, any> {
    ...
    if (c.lifecycle.preStop || c.lifecycle.postStart) {
      ...
      if (c.lifecycle.preStop) {
        if (c.lifecycle.preStop.type === 'exec')
          container.lifecycle.preStop = {
            exec: {
              command: c.lifecycle.preStop.execCommand
                .split('\n')
                .map((s) => s.trim())
                .filter(Boolean),
            },
          }
        else if (c.lifecycle.preStop.type === 'httpGet')
          container.lifecycle.preStop = {
            httpGet: {
              path: c.lifecycle.preStop.httpGetPath,
              port: c.lifecycle.preStop.httpGetPort,
            },
          }
        else if (c.lifecycle.preStop.type === 'tcpSocket')
          container.lifecycle.preStop = { tcpSocket: { port: c.lifecycle.preStop.tcpSocketPort } }
      }
      if (c.lifecycle.postStart) {
        if (c.lifecycle.postStart.type === 'exec')
          container.lifecycle.postStart = {
            exec: {
              command: c.lifecycle.postStart.execCommand
                .split('\n')
                .map((s) => s.trim())
                .filter(Boolean),
            },
          }
        else if (c.lifecycle.postStart.type === 'httpGet')
          container.lifecycle.postStart = {
            httpGet: {
              path: c.lifecycle.postStart.httpGetPath,
              port: c.lifecycle.postStart.httpGetPort,
            },
          }
        else if (c.lifecycle.postStart.type === 'tcpSocket')
          container.lifecycle.postStart = {
            tcpSocket: { port: c.lifecycle.postStart.tcpSocketPort },
          }
      }
    }
    const sc: any = {}
    ...
    if (
      c.securityContext.capabilitiesAdd.length > 0 ||
      c.securityContext.capabilitiesDrop.length > 0
    ) {
      sc.capabilities = {}
      if (c.securityContext.capabilitiesAdd.length > 0)
        sc.capabilities.add = c.securityContext.capabilitiesAdd.filter(Boolean)
      if (c.securityContext.capabilitiesDrop.length > 0)
        sc.capabilities.drop = c.securityContext.capabilitiesDrop.filter(Boolean)
    }
  ...
  if (form.podAffinityRules.length > 0) {
    const preferred = form.podAffinityRules
      .filter((r) => r.weight > 0 && r.topologyKey)
      .map((r) => ({ weight: r.weight, podAffinityTerm: buildAffinityTerm(r) }))
    const required = form.podAffinityRules
      .filter((r) => r.weight === 0 && r.topologyKey)
      .map((r) => buildAffinityTerm(r))
    if (!podSpec.affinity) podSpec.affinity = {}
    if (preferred.length > 0 || required.length > 0) {
      podSpec.affinity.podAffinity = {}
      if (preferred.length > 0)
        podSpec.affinity.podAffinity.preferredDuringSchedulingIgnoredDuringExecution = preferred
      if (required.length > 0)
        podSpec.affinity.podAffinity.requiredDuringSchedulingIgnoredDuringExecution = required
    }
  }
  if (form.podAntiAffinityRules.length > 0) {
    const preferred = form.podAntiAffinityRules
      .filter((r) => r.weight > 0 && r.topologyKey)
      .map((r) => ({ weight: r.weight, podAffinityTerm: buildAffinityTerm(r) }))
    const required = form.podAntiAffinityRules
      .filter((r) => r.weight === 0 && r.topologyKey)
      .map((r) => buildAffinityTerm(r))
    if (!podSpec.affinity) podSpec.affinity = {}
    if (preferred.length > 0 || required.length > 0) {
      ...
      if (preferred.length > 0)
        podSpec.affinity.podAntiAffinity.preferredDuringSchedulingIgnoredDuringExecution = preferred
      if (required.length > 0)
        podSpec.affinity.podAntiAffinity.requiredDuringSchedulingIgnoredDuringExecution = required
    }
  }
  ...
  if (props.kind === 'Deployment') {
    resource.spec = {
      ...
      strategy: { type: form.strategyType },
    }
    ...
    resource.spec = {
      ...
      updateStrategy: { type: form.updateStrategy },
      podManagementPolicy: form.podManagementPolicy,
    }
    const vcts = form.volumeClaimTemplates
      .filter((v) => v.name)
      .map((v) => {
        const vct: any = {
          metadata: { name: v.name },
          spec: { accessModes: v.accessModes, resources: { requests: { storage: v.storageSize } } },
        }
    ...
    resource.spec = {
      selector: { matchLabels: selectorLabels },
      template: podTemplate,
      updateStrategy: { type: form.dsUpdateStrategy },
    }
  }
      ...
      await (
        props.kind === 'Deployment'
          ...
          : props.kind === 'StatefulSet'
            ? createStatefulSet
            : createDaemonSet
      )({ namespace: form.namespace, yaml: generatedYaml.value })
  ...
  <div class="workload-form">
    <el-form ref="formRef" :model="form" :rules="formRules" label-position="top">
      ...
      <div class="form-section">
        ...
        <div class="section-content">
          <LabelsAnnotationsForm
            v-model:labels="form.labels"
            v-model:annotations="form.annotations"
          />
        </div>
      </div>
      ...
      <div class="form-section">
        ...
        <div class="section-content">
          ...
          <ContainerConfigForm
            ...
            title="初始化容器"
            :show-pull-policy="false"
            :min-containers="0"
            index-color="var(--el-color-warning)"
          />
        </div>
      </div>
      ...
      <div class="form-section">
        ...
        <div class="section-content">
          <StorageConfigForm
            v-model:volumes="form.volumes"
            v-model:volume-claim-templates="form.volumeClaimTemplates"
        ...
        <div class="section-content">
          ...
          <template v-if="kind === 'DaemonSet'">
            ...
            <el-form-item label="主机网络">
              ...
              <div class="form-help">
                启用后 Pod 直接使用主机网络命名空间，常用于需要访问主机网络的 DaemonSet（如监控
                agent、日志收集器）
              </div>
            </el-form-item>
          </template>
          ...
          <div class="fields-grid">
            <el-form-item label="DNS 策略">
              <el-select v-model="form.dnsPolicy" style="width: 100%">
                ...
                <el-option
                  label="ClusterFirstWithHostNet - 主机网络下仍用集群 DNS"
            ...
            <el-form-item label="优先级类名 (PriorityClassName)">
              ...
              <div class="form-help">
                如 system-node-critical, system-cluster-critical 等，决定资源不足时的驱逐顺序
              </div>
            </el-form-item>
