        ...
        <div style="margin-top: 12px">
          <div style="margin-bottom: 12px">
            <div style="display: flex; align-items: center; gap: 12px; margin-bottom: 8px">
              <span style="font-size: 13px; font-weight: 600; color: var(--el-text-color-regular)"
                >postStart（启动后）</span
              >
              <el-switch
                :model-value="!!container.lifecycle.postStart"
                @update:model-value="
                  (v: boolean) =>
                    v
                      ? (container.lifecycle.postStart = newLifecycleHandler())
                      : (container.lifecycle.postStart = null)
                "
              />
            </div>
            <template v-if="container.lifecycle.postStart">
              <div class="fields-grid">
                ...
                <el-form-item v-if="container.lifecycle.postStart.type === 'exec'" label="命令">
                  <el-input
                    v-model="container.lifecycle.postStart.execCommand"
                    type="textarea"
                ...
                <el-form-item v-if="container.lifecycle.postStart.type === 'httpGet'" label="端口">
                  <el-input-number
                    v-model="container.lifecycle.postStart.httpGetPort"
                    :min="1"
                    :max="65535"
                    style="width: 100%"
                  />
                ...
                <el-form-item
                  v-if="container.lifecycle.postStart.type === 'tcpSocket'"
                  label="端口"
                >
                  <el-input-number
                    v-model="container.lifecycle.postStart.tcpSocketPort"
                    :min="1"
                    :max="65535"
                    style="width: 100%"
                  />
          ...
          <div>
            <div style="display: flex; align-items: center; gap: 12px; margin-bottom: 8px">
              ...
              >
              <el-switch
                :model-value="!!container.lifecycle.preStop"
                @update:model-value="
                  (v: boolean) =>
                    v
                      ? (container.lifecycle.preStop = newLifecycleHandler())
                      : (container.lifecycle.preStop = null)
                "
              />
            </div>
            <template v-if="container.lifecycle.preStop">
              <div class="fields-grid">
                ...
                <el-form-item v-if="container.lifecycle.preStop.type === 'exec'" label="命令">
                  <el-input
                    v-model="container.lifecycle.preStop.execCommand"
                    type="textarea"
                    :autosize="{ minRows: 1, maxRows: 4 }"
                    placeholder="每行一个参数"
                  />
                ...
                <el-form-item v-if="container.lifecycle.preStop.type === 'httpGet'" label="端口">
                  <el-input-number
                    v-model="container.lifecycle.preStop.httpGetPort"
                    :min="1"
                    :max="65535"
                    style="width: 100%"
                  />
                ...
                <el-form-item v-if="container.lifecycle.preStop.type === 'tcpSocket'" label="端口">
                  <el-input-number
                    v-model="container.lifecycle.preStop.tcpSocketPort"
                    :min="1"
                    :max="65535"
                    style="width: 100%"
                  />
