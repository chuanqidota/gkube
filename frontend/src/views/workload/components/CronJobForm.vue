...
function parseContainer(c: any): typeof form.containers[0] {
  return {
    ...
    resources: { requests: { cpu: c.resources?.requests?.cpu || '', memory: c.resources?.requests?.memory || '' }, limits: { cpu: c.resources?.limits?.cpu || '', memory: c.resources?.limits?.memory || '' } },
    volumeMounts: (c.volumeMounts || []).map((m: any) => ({ name: m.name || '', mountPath: m.mountPath || '', subPath: m.subPath || '', readOnly: m.readOnly || false })),
    livenessProbe: parseProbe(c.livenessProbe), readinessProbe: parseProbe(c.readinessProbe), startupProbe: parseProbe(c.startupProbe),
    ...
    securityContext: { runAsUser: c.securityContext?.runAsUser ?? null, runAsNonRoot: c.securityContext?.runAsNonRoot || false, readOnlyRootFilesystem: c.securityContext?.readOnlyRootFilesystem || false, privileged: c.securityContext?.privileged || false, capabilitiesAdd: c.securityContext?.capabilities?.add || [], capabilitiesDrop: c.securityContext?.capabilities?.drop || [] },
  }
}
...
function parseProbe(probeData: any): Probe | null {
  ...
  return {
    ...
    httpGetPath: probeData.httpGet?.path || '/', httpGetPort: probeData.httpGet?.port || 80,
    tcpSocketPort: probeData.tcpSocket?.port || null, execCommand: probeData.exec?.command?.join('\n') || '',
    initialDelaySeconds: probeData.initialDelaySeconds || 15, periodSeconds: probeData.periodSeconds || 10,
    timeoutSeconds: probeData.timeoutSeconds || 5, failureThreshold: probeData.failureThreshold || 3,
  }
}
...
function parseLifecycleHandler(data: any): LifecycleHandler | null {
  ...
  return {
    ...
    httpGetPort: data.httpGet?.port || 80, tcpSocketPort: data.tcpSocket?.port || null,
  }
}
...
function parseInitialData(data: any) {
  ...
  form.volumes = volumes.map((v: any) => ({
    ...
    hostPath: v.hostPath?.path || '', hostPathType: v.hostPath?.type || 'DirectoryOrCreate',
    configMapName: v.configMap?.name || '', secretName: v.secret?.secretName || '', pvcName: v.persistentVolumeClaim?.claimName || '',
  }))
  ...
  form.tolerations = tolerations.map((t: any) => ({ key: t.key || '', operator: t.operator || 'Equal', value: t.value || '', effect: t.effect || 'NoSchedule', tolerationSeconds: t.tolerationSeconds || null }))
  ...
  const parseAffinityRules = (rules: any[]): AffinityRule[] =>
    (rules || []).map((r: any) => ({
      weight: r.weight || 1, topologyKey: r.podAffinityTerm?.topologyKey || r.topologyKey || '',
      namespaces: r.namespaces?.join(', ') || '',
      labelKey: r.labelSelector?.matchExpressions?.[0]?.key || '', labelValue: r.labelSelector?.matchExpressions?.[0]?.values?.[0] || '',
    }))
  ...
  form.topologySpreadConstraints = (podSpec.topologySpreadConstraints || []).map((t: any) => ({
    maxSkew: t.maxSkew || 1, topologyKey: t.topologyKey || '', whenUnsatisfiable: t.whenUnsatisfiable || 'DoNotSchedule',
    labelKey: t.labelSelector?.matchExpressions?.[0]?.key || '', labelValue: t.labelSelector?.matchExpressions?.[0]?.values?.[0] || '',
  }))
}
...
function buildK8sResource(): Record<string, any> {
  ...
  const resource: any = {
    ...
    spec: { schedule: form.schedule, concurrencyPolicy: form.concurrencyPolicy, suspend: form.suspend, jobTemplate: { spec: jobSpec } },
  }

  if (form.successfulJobsHistoryLimit != null) resource.spec.successfulJobsHistoryLimit = form.successfulJobsHistoryLimit
  if (form.failedJobsHistoryLimit != null) resource.spec.failedJobsHistoryLimit = form.failedJobsHistoryLimit
  if (form.startingDeadlineSeconds != null) resource.spec.startingDeadlineSeconds = form.startingDeadlineSeconds
...
function parseCpuToMillicores(cpu: string): number | null {
  if (!cpu) return null; cpu = cpu.trim()
  if (cpu.endsWith('m')) return parseInt(cpu.slice(0, -1), 10)
  const val = parseFloat(cpu); return isNaN(val) ? null : Math.round(val * 1000)
}

function parseMemoryToBytes(mem: string): number | null {
  if (!mem) return null; mem = mem.trim()
  const units: Record<string, number> = { 'Ki': 1024, 'Mi': 1024**2, 'Gi': 1024**3, 'Ti': 1024**4, 'K': 1000, 'M': 1000**2, 'G': 1000**3, 'T': 1000**4 }
  for (const [suffix, multiplier] of Object.entries(units)) {
    if (mem.endsWith(suffix)) { const val = parseFloat(mem.slice(0, -suffix.length)); return isNaN(val) ? null : Math.round(val * multiplier) }
  }
  const val = parseFloat(mem); return isNaN(val) ? null : val
}
