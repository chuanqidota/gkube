...
export function useFormArrays(formData: {
  ...
  const removeContainer = (i: number) => {
    if (formData.containers.length > 1) formData.containers.splice(i, 1)
  }
  ...
  const addPort = (ci: number, isInit?: boolean) => {
    ;(isInit ? formData.initContainers[ci] : formData.containers[ci]).ports.push({
      name: '',
      containerPort: null,
      protocol: 'TCP',
    })
  ...
  const addVolumeMount = (ci: number, isInit?: boolean) => {
    ;(isInit ? formData.initContainers[ci] : formData.containers[ci]).volumeMounts.push({
      name: '',
      mountPath: '',
      subPath: '',
      readOnly: false,
  ...
  const enableLifecycle = (ci: number, hookType: 'preStop' | 'postStart', isInit?: boolean) => {
    ;(isInit ? formData.initContainers[ci] : formData.containers[ci]).lifecycle[hookType] =
      createEmptyLifecycleHandler()
  }
  ...
  const addVolumeClaimTemplate = () =>
    formData.volumeClaimTemplates?.push({
      name: '',
      storageSize: '1Gi',
      storageClassName: '',
