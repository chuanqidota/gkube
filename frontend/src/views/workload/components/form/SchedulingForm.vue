...
const topologySpreadConstraints = defineModel<TopologySpreadConstraint[]>('topologySpreadConstraints', { required: true })
...
function addToleration() {
  tolerations.value.push({ key: '', operator: 'Equal', value: '', effect: 'NoSchedule', tolerationSeconds: null })
}
...
function addAffinityRule(type: 'podAffinity' | 'podAntiAffinity') {
  ...
  list.value.push({ weight: 1, topologyKey: 'kubernetes.io/hostname', namespaces: '', labelKey: '', labelValue: '' })
}
...
function addTopologySpreadConstraint() {
  topologySpreadConstraints.value.push({ maxSkew: 1, topologyKey: 'kubernetes.io/hostname', whenUnsatisfiable: 'DoNotSchedule', labelKey: '', labelValue: '' })
}
