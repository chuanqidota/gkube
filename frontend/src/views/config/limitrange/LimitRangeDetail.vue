...
function handleEdit() {
  ...
  if (!yamlContent.value) {
    yamlLoading.value = true
    getLimitRangeYaml({ namespace, name })
      .then((res: any) => {
          ...
          <el-card shadow="never">
            <el-descriptions :column="2" border>
              <el-descriptions-item label="Name">{{
                limitRange.name || limitRange.metadata?.name
              }}</el-descriptions-item>
              <el-descriptions-item label="Namespace">{{
                limitRange.namespace || limitRange.metadata?.namespace
              }}</el-descriptions-item>
              <el-descriptions-item label="Age">{{
                limitRange.age || limitRange.metadata?.creationTimestamp || '-'
              }}</el-descriptions-item>
              <el-descriptions-item label="UID">{{
                limitRange.metadata?.uid || '-'
              }}</el-descriptions-item>
            </el-descriptions>
          </el-card>
            ...
            <el-table :data="limitRange.limits || limitRange.spec?.limits || []" border stripe>
              ...
              <el-table-column label="Max" min-width="200">
                <template #default="{ row }">
                  <div v-for="(v, k) in row.max || {}" :key="k" style="font-size: 12px">
                    {{ k }}: {{ v }}
                  </div>
              ...
              <el-table-column label="Min" min-width="200">
                <template #default="{ row }">
                  <div v-for="(v, k) in row.min || {}" :key="k" style="font-size: 12px">
                    {{ k }}: {{ v }}
                  </div>
              ...
              <el-table-column label="Default" min-width="200">
                <template #default="{ row }">
                  <div v-for="(v, k) in row.default || {}" :key="k" style="font-size: 12px">
                    {{ k }}: {{ v }}
                  </div>
              ...
              <el-table-column label="Default Request" min-width="200">
                <template #default="{ row }">
                  <div v-for="(v, k) in row.defaultRequest || {}" :key="k" style="font-size: 12px">
                    {{ k }}: {{ v }}
                  </div>
                  <span v-if="!row.defaultRequest || Object.keys(row.defaultRequest).length === 0"
                    >-</span
                  >
                </template>
              </el-table-column>
              <el-table-column label="Max Limit/Request Ratio" min-width="180">
                <template #default="{ row }">
                  <div
                    v-for="(v, k) in row.maxLimitRequestRatio || {}"
                    :key="k"
                  ...
                  <span
                    v-if="
                      ...
                      Object.keys(row.maxLimitRequestRatio).length === 0
                    "
                    >-</span
                  >
                </template>
