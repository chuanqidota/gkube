...
import { createSecret } from '@/api/resource'
...
type: Opaque
data:
  # value must be base64-encoded, e.g. "value" -> dmFsdWU=
  key: dmFsdWU=
`)
...
async function handleYamlSubmit() {
  ...
  try {
    ...
    await createSecret({ namespace: ns, yaml: yamlContent.value })
...
<style scoped>
.secret-create { max-width: 1100px; margin: 0 auto; padding: 20px 0; }
