...
import { createVpa } from '@/api/resource'

const router = useRouter()
const { t } = useI18n()
const mode = ref<'form' | 'yaml'>('form')
const yamlEditorRef = ref()
const yamlContent = ref(`apiVersion: autoscaling.k8s.io/v1
...
async function handleYamlSubmit() {
  ...
  try {
    ...
    await createVpa({ namespace: ns, yaml: yamlContent.value })
...
<template>
  <div class="vpa-create">
    ...
    <VPAForm v-if="mode === 'form'" />

    <div v-else class="yaml-mode">
      <el-alert
        title="默认使用 updateMode: Off，仅生成资源推荐，不会自动驱逐或重建 Pod。"
        type="info"
        show-icon
        :closable="false"
        class="mode-tip"
      ...
      <div class="yaml-card">
        ...
        <div class="yaml-card-body">
          <YamlEditor ref="yamlEditorRef" v-model="yamlContent" height="calc(100vh - 220px)" :read-only="false" editable auto-format :show-toolbar="false" title="YAML 配置">
...
<style scoped>
.vpa-create { max-width: 1100px; margin: 0 auto; padding: 20px 0; }
