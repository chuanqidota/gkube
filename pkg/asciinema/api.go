...
// WriteHeader 写头部信息
func WriteHeader(key string, cols, rows int, startTime time.Time, record *audit.EsRecord) {
	header := RecHeader{
		Version:   2,
		Width:     cols,
		Height:    rows,
...
// WriteData 写入数据
func WriteData(key string, startTime time.Time, out string, record *audit.EsRecord) {
...
// WriteSize 写尺寸数据
func WriteSize(key string, startTime time.Time, cols, rows uint16, record *audit.EsRecord) {
	sub := float64(time.Since(startTime).Microseconds()) / float64(1000000)
